...
function vuelo_Una_Particula_Obstaculos(Posicion, Velocidad, Tiempo_Vuelo, Indice_Obstaculo_Cercano, Voronoi, Diccionario_Vertices_Indices, Diccionario_Vertices_Cuadrado, Lado_Poligono_Bloqueado, Array_Free_Fly, Obstacle_A, Obstacle_B, Initial_Position_Obstacle, Initial_Velocity_Obstacle, funcion_Cambio_Celda::Function, funcion_Estado_Tras_Tiempo::Function)
    while Tiempo_Vuelo > 0.0
    	#Finaliza el código tras alcanzar 1 colisión, mandando un tiempo restante "Infinito" únicamente alcanzable con esta condición
    	if length(Array_Free_Fly) == 1
        ...
        Posicion_Cambio_Celda, Velocidad_Cambio_Celda, Tiempo_Vuelo_Cambio, Lado_Poligono_Bloqueado_Cambio, Posicion_Colision_B = funcion_Cambio_Celda(Posicion_Inicial_Cambio, Velocidad_Inicial_Cambio, Arreglo_Vertices_Contenedor, Lado_Poligono_Bloqueado_Cambio, Posicion_Obstaculo, Diccionario_Segmentos_Centro_Vecino);
        ...
        if (Tiempo_Vuelo - Tiempo_Vuelo_Cambio) > 0.0 #Hay tiempo para que la partícula salga del polígono
            ...
            if Posicion_Colision_B == [Inf, Inf]
                nothing;
            else
                Obstacle_B = copy(Posicion_Colision_B);
                Longitud_Vuelo_Libre = norm(Obstacle_B - Obstacle_A);
                push!(Array_Free_Fly, Longitud_Vuelo_Libre);
                Obstacle_A = copy(Posicion_Colision_B);
                Initial_Position_Obstacle = copy(Posicion_Colision_B)
                Initial_Velocity_Obstacle = copy(Velocidad_Cambio_Celda)
            end
        else #No hay tiempo de vuelo restante para salir del polígono
            Posicion, Velocidad, Lado_Poligono_Bloqueado, Posicion_Colision_B = funcion_Estado_Tras_Tiempo(Posicion, Velocidad, Arreglo_Vertices_Contenedor, Lado_Poligono_Bloqueado, Posicion_Obstaculo, Diccionario_Segmentos_Centro_Vecino, Tiempo_Vuelo);
            ...
            if Posicion_Colision_B == [Inf, Inf]
                ...
                Obstacle_A = copy(Posicion_Colision_B);
                Initial_Position_Obstacle = copy(Posicion_Colision_B)
                Initial_Velocity_Obstacle = copy(Velocidad)
            end
