...
function vuelo_Una_Particula_Obstaculos(Posicion, Velocidad, Tiempo_Vuelo, Indice_Obstaculo_Cercano, Voronoi, Diccionario_Vertices_Indices, Diccionario_Vertices_Cuadrado, Lado_Poligono_Bloqueado, funcion_Cambio_Celda::Function, funcion_Estado_Tras_Tiempo::Function)
    ...
    while Tiempo_Vuelo > 0.0
        Contador += 1;
        #println("Estamos calculando la iteración $(Contador)")
        ...
        Arreglo_Vertices_Celda_Contenedora = []; ##V-ONLY##
        push!(Central_Obstacles, Posicion_Obstaculo); ##V-ONLY##
        #println("El obstáculo está centrado en $(Posicion_Obstaculo)")
        push!(Positions_Particle, Posicion); ##V-ONLY##
        #println("La partícula está en la posición $(Posicion)")
        push!(Velocities_Particle, Velocidad); ##V-ONLY##
        #println("La partícula tiene velocidad $(Velocidad)")
        push!(Vertices_Door, Lado_Poligono_Bloqueado); ##V-ONLY##
        #println("El lado bloqueado para la partícula es $(Lado_Poligono_Bloqueado)")
        ...
        push!(Dictionary_Arrays_Centro_Vecino, Diccionario_Segmentos_Centro_Vecino); ##V-ONLY##
        push!(Dictionary_Arrays_Indice, Diccionario_Segmento_Indice); ##V-ONLY##
        push!(Vertices_Voronoi, Arreglo_Vertices_Contenedor); ##V-ONLY##
        ...
        #println("La posición de cambio de celda es $(Posicion_Cambio_Celda)")
        #println("La velocidad de cambio de celda es $(Velocidad_Cambio_Celda)")
        #println("El tiempo de vuelo requerido es $(Tiempo_Vuelo_Cambio)")
...
function MSD_Varying_Velocities(Patch_Information, Reduction_Factor, Average_Distance_Stripes, Star_Vectors, Alphas_Array, APoint_Initial, Number_Velocities, Number_Flights, Short_Fly_Time, MSD_Array, funcion_Cambio_Celda::Function, funcion_Estado_Tras_Tiempo::Function)
    ...
    for α in 1:Number_Velocities
        ...
        #AVelocity_Initial = [0.28507222746899546, -0.9585060381269725]; ##V-ONLY## (Primer éxito)
        #AVelocity_Initial = [-0.9159100425284007, 0.4013835995598516]; ##V-ONLY## (Trayectoria muerta)
        #AVelocity_Initial = [-0.8018436591360344, 0.597533887158992];
        AVelocity = copy(AVelocity_Initial);
        APoint = copy(APoint_Initial); #A copy of Initial Position in order to preserve the original one
