#Función que nos regresa el número de lados de los polígonos de Voronoi de los clúster principales (antes de remover 
#las capas externas para evitar deformar los polígonos cercanos a la última capa del cluster).
#La función se itera un cierto número de veces para tener una mayor estadística.
#"Iteraciones" es el número de clúster principales que vamos a considerar para el número de lados.
#"Iteraciones_Cluster" es el número de iteraciones del algoritmo de acotar polígonos de Voronoi por su área que se
#empleará para determinar un clúster principal
#"Area_Cota" es el área que servirá como discriminante de buenos y malos polígonos en el algoritmo de acotar poligonos
#"Margen_Error" es el parámetro que determina qué margen de error se considera al generar los posibles números enteros
#asociados al polígono de Voronoi que contiene al punto arbitrario.
#"Semilado_Caja" es el semilado de un cuadrado en el cual se genera un punto arbitrario, alrededor del cual se genera
#la vecindad del arreglo cuasiperiódico.
function arreglo_Numero_Lados_Buenos_Poligonos(Iteraciones, Iteraciones_Cluster, Area_Cota, Margen_Error, Semilado_Caja)
            ...
            if Face.site in Centroides_Iterados #Si es cierto, el polígono es uno de nuestros polígonos de interés
                ...
                while Halfedge.next != Face.outerComponent
                    ...
                    if Vertice in Arreglo_Coordenadas_Vertices
                        ...
                        #Comparamos con el vértice previo para ver si el lado que se añade es demasiado pequeño, si es
                        #el caso lo quitamos.
