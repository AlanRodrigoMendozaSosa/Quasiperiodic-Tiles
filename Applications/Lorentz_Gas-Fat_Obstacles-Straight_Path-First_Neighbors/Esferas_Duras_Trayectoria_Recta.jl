...
function cambio_Celda_Esferas_Duras_Trayectoria_Recta(Posicion_Inicial, Velocidad_Inicial, Arreglo_Vertices_Celda, Coordenadas_Segmento_Entrada, Posicion_Obstaculo, Posiciones_Obstaculos_Vecinos, Diccionario_Segmentos_Centro_Vecino)
	Tiempo_Acumulado_Vuelo = 0; #Variable que contendrá el tiempo de vuelo que llevamos acumulado
    Posicion_Colisiones = []; #Posiciones de las colisiones con los obstáculos
    Velocidad_Colisiones = []; #Velocidades de las colisiones con los obstáculos

    while true
        #PRIMEROS VECINOS ONLY
        Arreglo_Tiempos_Colision_Obstaculos = []; #Arreglo con los tiempos que tarda en colisionar la partícula con cada uno de los obstáculos
        Diccionario_Tiempo_Posicion = Dict();
        Diccionario_Tiempo_Velocidad = Dict();

        #Tiempo de colisión con el obstáculo "central", es decir, el obstáculo cuyo centro es el sitio asociado a la celda de Voronoi contenedora
    	Posicion_Colision_Central, Velocidad_Colision_Central, Tiempo_Vuelo_Colision_Central = colision_Obstaculo(Posicion_Inicial, Velocidad_Inicial, Radio_Obstaculo, Posicion_Obstaculo, colision_Esfera_Dura, velocidad_Esfera_Dura);
        push!(Arreglo_Tiempos_Colision_Obstaculos, Tiempo_Vuelo_Colision_Central); #PRIMEROS VECINOS ONLY
        Diccionario_Tiempo_Posicion[Tiempo_Vuelo_Colision_Central] = Posicion_Colision_Central; #PRIMEROS VECINOS ONLY
        Diccionario_Tiempo_Velocidad[Tiempo_Vuelo_Colision_Central] = Velocidad_Colision_Central; #PRIMEROS VECINOS ONLY

        #PRIMEROS VECINOS ONLY
        #Tiempos de colisión con los obstáculos de Primeros Vecinos
        for Centro_Vecino in Posiciones_Obstaculos_Vecinos
            Posicion_Colision_Vecino, Velocidad_Colision_Vecino, Tiempo_Vuelo_Colision_Vecino = colision_Obstaculo(Posicion_Inicial, Velocidad_Inicial, Radio_Obstaculo, [Centro_Vecino[1], Centro_Vecino[2]], colision_Esfera_Dura, velocidad_Esfera_Dura);
            push!(Arreglo_Tiempos_Colision_Obstaculos, Tiempo_Vuelo_Colision_Vecino); #PRIMEROS VECINOS ONLY
            Diccionario_Tiempo_Posicion[Tiempo_Vuelo_Colision_Vecino] = Posicion_Colision_Vecino; #PRIMEROS VECINOS ONLY
            Diccionario_Tiempo_Velocidad[Tiempo_Vuelo_Colision_Vecino] = Velocidad_Colision_Vecino; #PRIMEROS VECINOS ONLY
        end

        #Nos quedamos con el evento de colisión que ocurre al menor tiempo
        Tiempo_Vuelo_Colision = minimum(Arreglo_Tiempos_Colision_Obstaculos);
        Posicion_Colision = Diccionario_Tiempo_Posicion[Tiempo_Vuelo_Colision];
        Velocidad_Colision = Diccionario_Tiempo_Velocidad[Tiempo_Vuelo_Colision];

        #Tiempo de colisión con los segmentos de la celda de Voronoi contenedora
    	...
    	if Tiempo_Vuelo_Colision < Tiempo_Vuelo_Cambio_Celda #La partícula colisiona antes de moverse de polígono
            ...
            push!(Posicion_Colisiones, Posicion_Colision);
            push!(Velocidad_Colisiones, Velocidad_Colision);
            ...
            return Posicion_Cambio_Celda, Velocidad_Cambio_Celda, Tiempo_Acumulado_Vuelo, Extremos_Cambio_Celda, Posicion_Colisiones, Velocidad_Colisiones
        end
    end
end
...
function estado_Tras_Tiempo_Rectas(Posicion_Inicial, Velocidad_Inicial, Arreglo_Vertices_Celda, Coordenadas_Segmento_Entrada, Posicion_Obstaculo, Posiciones_Obstaculos_Vecinos, Diccionario_Segmentos_Centro_Vecino, Tiempo_Vuelo_Parcial)
	Posicion_Colisiones = []; #Posiciones de las colisiones con los obstáculos
    Velocidad_Colisiones = []; #Velocidades de las colisiones con los obstáculos
    ...
    while true
        ...
        #Calculamos las condiciones de colisión y el tiempo de colisión con el obstáculo central de la celda de Voronoi contenedora
        Posicion_Colision_Central, Velocidad_Colision_Central, Tiempo_Vuelo_Colision_Central = colision_Obstaculo(Posicion_Inicial, Velocidad_Inicial, Radio_Obstaculo, Posicion_Obstaculo, colision_Esfera_Dura, velocidad_Esfera_Dura);
        push!(Arreglo_Tiempos_Colision_Obstaculos, Tiempo_Vuelo_Colision_Central); #PRIMEROS VECINOS ONLY
        Diccionario_Tiempo_Posicion[Tiempo_Vuelo_Colision_Central] = Posicion_Colision_Central; #PRIMEROS VECINOS ONLY
        Diccionario_Tiempo_Velocidad[Tiempo_Vuelo_Colision_Central] = Velocidad_Colision_Central; #PRIMEROS VECINOS ONLY
        ...
        #PRIMEROS VECINOS ONLY
        #Calculamos las condiciones de colisión y el tiempo de colisión con los obstáculos vecinos a la celda de Voronoi contenedora
        for Centro_Vecino in Posiciones_Obstaculos_Vecinos
            Posicion_Colision_Vecino, Velocidad_Colision_Vecino, Tiempo_Vuelo_Colision_Vecino = colision_Obstaculo(Posicion_Inicial, Velocidad_Inicial, Radio_Obstaculo, [Centro_Vecino[1], Centro_Vecino[2]], colision_Esfera_Dura, velocidad_Esfera_Dura);
            push!(Arreglo_Tiempos_Colision_Obstaculos, Tiempo_Vuelo_Colision_Vecino); #PRIMEROS VECINOS ONLY
            Diccionario_Tiempo_Posicion[Tiempo_Vuelo_Colision_Vecino] = Posicion_Colision_Vecino; #PRIMEROS VECINOS ONLY
        ...
        #Calculamos el tiempo que le toma a la partícula salir por alguna de las paredes de la celda de Voronoi
    	Posicion_Cambio_Celda, Velocidad_Cambio_Celda, Tiempo_Vuelo_Cambio_Celda, Extremos_Cambio_Celda = colision_Segmento(Posicion_Inicial, Velocidad_Inicial, Arreglo_Vertices_Celda, Coordenadas_Segmento_Entrada, Posicion_Obstaculo, Diccionario_Segmentos_Centro_Vecino, interseccion_Particula_Celda_Trayectorias_Rectas, velocidad_Cambio_Celda_Trayectorias_Rectas);

    	if Tiempo_Vuelo_Colision < Tiempo_Vuelo_Cambio_Celda #La partícula presenta una colisión antes de moverse de polígono
            if (Tiempo_Vuelo_Parcial - Tiempo_Vuelo_Colision) > 0.0 #Hay tiempo para la colisión completa
    			...
    			return Posicion_Inicial, Velocidad_Inicial, Coordenadas_Segmento_Entrada, Posicion_Colisiones, Velocidad_Colisiones
            end
        else #La partícula se mueve de polígono antes de colisionar con el obstáculo de su polígono contenedor
