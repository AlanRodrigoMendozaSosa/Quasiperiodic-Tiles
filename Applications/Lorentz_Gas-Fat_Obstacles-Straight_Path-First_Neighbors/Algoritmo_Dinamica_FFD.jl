...
function vuelo_Una_Particula_Obstaculos(Posicion, Velocidad, Tiempo_Vuelo, Indice_Obstaculo_Cercano, Voronoi, Diccionario_Vertices_Indices, Diccionario_Vertices_Cuadrado, Lado_Poligono_Bloqueado, Array_Free_Fly, Obstacle_A, Obstacle_B, Initial_Position_Obstacle, Initial_Velocity_Obstacle, funcion_Cambio_Celda::Function, funcion_Estado_Tras_Tiempo::Function)
    while Tiempo_Vuelo > 0.0
    	#Finaliza el código tras alcanzar al menos una colisión, mandando un tiempo restante "Infinito" únicamente alcanzable con esta condición
    	if length(Array_Free_Fly) != 0
    		return Posicion, Velocidad, Inf, Indice_Obstaculo_Cercano, Lado_Poligono_Bloqueado, Obstacle_A, Obstacle_B, Initial_Position_Obstacle, Initial_Velocity_Obstacle
    	end
        ...
        if Diccionario_Vertices_Cuadrado[Voronoi.faces[Indice_Obstaculo_Cercano].site] == false
            return Posicion, Velocidad, Tiempo_Vuelo, Indice_Obstaculo_Cercano, Lado_Poligono_Bloqueado, Obstacle_A, Obstacle_B, Initial_Position_Obstacle, Initial_Velocity_Obstacle
        end
        ...
        #PRIMEROS VECINOS ONLY
        Arreglo_Centro_Obstaculos_Vecinos = []; #Arreglo con las coordenadas X,Y de los obstáculos vecinos a la celda de Voronoi que contiene al punto.
        ...
        while true
            ...
            push!(Arreglo_Centro_Obstaculos_Vecinos, Obstaculo_Vecino); #Agrego las coordenadas del obstáculo vecino. #PRIMEROS VECINOS ONLY
        ...
        #Empleamos la función del usuario para que nos regrese la posición y velocidad de la partícula al salir del polígono contenedor actual, así como el tiempo de vuelo de en salir del
        #polígono y las coordenadas [[X1, Y1], [X2, Y2]] del segmento por el que sale la partícula
        Posicion_Cambio_Celda, Velocidad_Cambio_Celda, Tiempo_Vuelo_Cambio, Lado_Poligono_Bloqueado_Cambio, Posicion_Colisiones, Velocidad_Colisiones = funcion_Cambio_Celda(Posicion_Inicial_Cambio, Velocidad_Inicial_Cambio, Arreglo_Vertices_Contenedor, Lado_Poligono_Bloqueado_Cambio, Posicion_Obstaculo, Arreglo_Centro_Obstaculos_Vecinos, Diccionario_Segmentos_Centro_Vecino);
        ...
        if (Tiempo_Vuelo - Tiempo_Vuelo_Cambio) > 0.0 #Hay tiempo para que la partícula salga del polígono
            ...
            if length(Posicion_Colisiones) == 0 #No hubo ninguna colisión dentro de la celda contenedora
                nothing;
            else #Hubo por lo menos una colisión dentro de la celda contenedora
                for i in 1:length(Posicion_Colisiones)
                    Obstacle_B = Posicion_Colisiones[i];
                    Longitud_Vuelo_Libre = norm(Obstacle_B - Obstacle_A);
                    push!(Array_Free_Fly, Longitud_Vuelo_Libre);
                    Obstacle_A = Posicion_Colisiones[i];
                end
                Initial_Position_Obstacle = Posicion_Colisiones[end]
                Initial_Velocity_Obstacle = Velocidad_Colisiones[end]
            end
        else #No hay tiempo de vuelo restante para salir del polígono
            Posicion, Velocidad, Lado_Poligono_Bloqueado, Posicion_Colisiones, Velocidad_Colisiones = funcion_Estado_Tras_Tiempo(Posicion, Velocidad, Arreglo_Vertices_Contenedor, Lado_Poligono_Bloqueado, Posicion_Obstaculo, Arreglo_Centro_Obstaculos_Vecinos, Diccionario_Segmentos_Centro_Vecino, Tiempo_Vuelo);
            ...
            return Posicion, Velocidad, 0.0, Indice_Obstaculo_Cercano, Lado_Poligono_Bloqueado, Obstacle_A, Obstacle_B, Initial_Position_Obstacle, Initial_Velocity_Obstacle
        end
    end
end

#Function that, given a initial position in the Quasiperiodic Array, calculates the MSD of the system iterating over a given number of flies (each one conformed as a set of short-time flies)
#each one with a random velocity.
...
#APoint_Initial: A fixed position in the Quasiperiodic array that will be the Initial Position of all the particles, each one with a different random velocity.
#Obstacle_Radius: The radius of the obstacles centered in the vertices of the quasiperiodic array.
#funcion_Cambio_Celda: Function that calculates the position and velocity of a particle when it leaves the container polygon, as well as the time it requires to leave the polygon and the 
#coordinates of the vertices of the segment where it leaves.
#funcion_Estado_Tras_Tiempo: Function that calculates the position and velocity of a particle, as well as the vertices of the segment from which the particle enter in the latest move.
function first_Fly(Patch_Information, Reduction_Factor, Average_Distance_Stripes, Star_Vectors, Alphas_Array, APoint_Initial, Long_Fly_Time, Array_Free_Fly, Initial_Position_Obstacle, Initial_Velocity_Obstacle, funcion_Cambio_Celda::Function, funcion_Estado_Tras_Tiempo::Function)
    ...
    #STEP 4: Let's prepare some information about the trajectory of the particle
    Obstacle_A = copy(APoint_Initial); #Update the Position of the N-1 Obstacle as the Initial Position of the particle
    Obstacle_B = [0.0, 0.0]; #A variable corresponding to the position of the N Obstacle.
    ...
    Remaining_Fly_Time = Long_Fly_Time;

    try
        APoint,AVelocity,Remaining_Fly_Time,Nearest_Vertex_Index_Voronoi,Forbidden_Side_Polygon,Obstacle_A,Obstacle_B,Initial_Position_Obstacle,Initial_Velocity_Obstacle = vuelo_Una_Particula_Obstaculos(APoint,AVelocity,Long_Fly_Time,Nearest_Vertex_Index_Voronoi,Voronoi_Vertices,Dictionary_Vertex_Index,Dictionary_Vertices_Inside_Square,Forbidden_Side_Polygon,Array_Free_Fly,Obstacle_A,Obstacle_B,Initial_Position_Obstacle,Initial_Velocity_Obstacle,funcion_Cambio_Celda,funcion_Estado_Tras_Tiempo);
        if Remaining_Fly_Time == Inf #Condición que ocurre únicamente al colisionar la partícula con un obstáculo
            @goto Final_Codigo_Primer_Colisión #Finalizamos el código, para ello hemos colocado esta etiqueta que automáticamente manda al final del código
        end
    catch Err
    ...
    while Remaining_Fly_Time > 0.0
        ...
        Nuevo_Centro_Parche = copy(APoint);
        Array_Position_Patches = [Nuevo_Centro_Parche];
        ...
        APoint,AVelocity,Remaining_Fly_Time,Nearest_Vertex_Index_Voronoi,Forbidden_Side_Polygon,Obstacle_A,Obstacle_B,Initial_Position_Obstacle,Initial_Velocity_Obstacle = vuelo_Una_Particula_Obstaculos(APoint,AVelocity,Remaining_Fly_Time,Nearest_Vertex_Index_Voronoi,Voronoi_Vertices,Dictionary_Vertex_Index,Dictionary_Vertices_Inside_Square,Forbidden_Side_Polygon,Array_Free_Fly,Obstacle_A,Obstacle_B,Initial_Position_Obstacle,Initial_Velocity_Obstacle,funcion_Cambio_Celda,funcion_Estado_Tras_Tiempo);
        if Remaining_Fly_Time == Inf #Condición que ocurre únicamente al colisionar la partícula con X obstáculos (sin contar la primera colisión)
            @goto Final_Codigo_Primer_Colisión #Finalizamos el código, para ello hemos colocado esta etiqueta que automáticamente manda al final del código
        end
    ...
    @label Final_Codigo_Primer_Colisión
    return Initial_Position_Obstacle, Initial_Velocity_Obstacle
end

function multiple_Flies(Patch_Information, Reduction_Factor, Average_Distance_Stripes, Star_Vectors, Alphas_Array, APoint_Initial, Long_Fly_Time, Array_Free_Fly, Initial_Position_Obstacle, Initial_Velocity_Obstacle, funcion_Cambio_Celda::Function, funcion_Estado_Tras_Tiempo::Function)
    Average_Radius = Patch_Information[end]; #The value of the previously calculated "Average Radius"
    Safe_Radius = Reduction_Factor*Average_Radius; #The Safe Radius value
    ...
    #STEP 4: Let's define some variable related with the obstacles and the start of the free fly
    Obstacle_A = copy(APoint_Initial); #A variable to save the Position of the collision with the N-1 Obstacle
    Obstacle_B = [0.0, 0.0]; #A variable to save the Position of the collision with the N Obstacle

    Dead_Trajectory = false; #We suppose that the trajectory won't be a closed one

    #STEP 5: Get a Initial Velocity
    AVelocity = copy(Initial_Velocity_Obstacle); #Obtain the velocity of the particle after the collision with the N-1 obstacle
    ...
    try
        APoint,AVelocity,Remaining_Fly_Time,Nearest_Vertex_Index_Voronoi,Forbidden_Side_Polygon,Obstacle_A,Obstacle_B,Initial_Position_Obstacle,Initial_Velocity_Obstacle = vuelo_Una_Particula_Obstaculos(APoint,AVelocity,Long_Fly_Time,Nearest_Vertex_Index_Voronoi,Voronoi_Vertices,Dictionary_Vertex_Index,Dictionary_Vertices_Inside_Square,Forbidden_Side_Polygon,Array_Free_Fly,Obstacle_A,Obstacle_B,Initial_Position_Obstacle,Initial_Velocity_Obstacle,funcion_Cambio_Celda,funcion_Estado_Tras_Tiempo);
        if Remaining_Fly_Time == Inf #Condición que ocurre únicamente al colisionar la partícula con X obstáculos (sin contar la primera colisión)
            @goto Final_Codigo_Colisión #Finalizamos el código, para ello hemos colocado esta etiqueta que automáticamente manda al final del código
        end
    catch Err
        if isa(Err, DeadTrajectory)
            println("The particle $(α) is a dead one")
